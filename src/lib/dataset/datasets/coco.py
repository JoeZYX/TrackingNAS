...
class COCO(GenericDataset):
  ...
  # new valid_ids ---------> 不是用所有的
  # define new _valid_ids
  # -----------------+++++++++++++++++----------------------
  new_valid_ids = [1,2,3,4,6,8]
  #new_valid_ids = []
  if len(new_valid_ids) > 0 :
    cat_ids = {v: i + 1 for i, v in enumerate(new_valid_ids)}
    new_class_name = []
    for key in cat_ids.keys():
      new_class_name.append(class_name[cat_ids[key]])
    _valid_ids = new_valid_ids
