...
from .networks.super_network import SuperNetNetwork
from .networks.OneForAll_NAS import OneForAll_SuperNetNetwork
...
_network_factory = {
  ...
  'custom' : SuperNetNetwork,
  'oneforall':OneForAll_SuperNetNetwork
}

def create_model(arch, head, head_conv, opt=None):
  if arch == "oneforall":
    model_class = _network_factory[arch]
    print("build the oneforall model with configuration {}".format(opt.onceforall_config_path))
    with open(opt.onceforall_config_path, 'r') as yaml_file:
        network_config = yaml.load(yaml_file, Loader=yaml.FullLoader)
    model = OneForAll_SuperNetNetwork(network_config = network_config, opt=opt)
    return model
  elif arch == 'custom':
    model_class = _network_factory[arch]
    print("build the custom model with seed {}".format(opt.nectwork_seed))
    model = model_class(heads=head, nectwork_seed=opt.nectwork_seed, width_mult=opt.width_mult, opt=opt)
    return model
  else:
    ...
    print("================= arch : ", arch)
    model_class = _network_factory[arch]
    print(num_layers, head, head_conv)
