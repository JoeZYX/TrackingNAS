...
class Detector(object):
  def __init__(self, opt):
    ...
    if opt.load_model.endswith(".pth"):
        ...
        self.model.eval()
        self.modeltype = "pth"
    elif opt.load_model.endswith(".onnx"):
        opt.device = torch.device('cpu')
        ...
        self.ort_session = ort.InferenceSession(opt.load_model, sess_options)
        self.input_name_1 = self.ort_session.get_inputs()[0].name
        self.input_name_2 = self.ort_session.get_inputs()[1].name
        self.modeltype = "onnx"
    else:
        raise TypeError("Model type is wrong")
...
#         print(" scale : " , scale)
        #print("image,size: ", image.shape[0:2])
        #print("===============================================")
        images, meta = self.pre_process(image, scale, meta) # 输入的 meta :  {}  scale :  1.0
      else:
        # prefetch testing
      ...
      #print(" post process :" ,dets.keys(), meta,scale)
  ...
  def _transform_scale(self, image, scale=1):
    ...
    # print("self.opt.fix_short : ", self.opt.fix_short)
    # print("self.opt.fix_res :" , self.opt.fix_res)
    
    # self.opt.fix_short -1
    # self.opt.fix_res : True
    #print("============================ self.opt.fix_short =============", self.opt.fix_short)
    if self.opt.fix_short > 0:
      #print("self.opt.fix_short")
      if height < width:
    ...
    # print( "==============================, inp_height   ",inp_height)
    # print( "==============================, inp_width    ",inp_width)
    # print( "==============================, c    ",c)
    # print( "==============================, s    ",s)
  ...
  def process(self, images, pre_images=None, pre_hms=None,
    ...
    with torch.no_grad():
      torch.cuda.synchronize()
      if self.modeltype == "pth":
          print(images.shape)
          output = self.model(images, pre_images, pre_hms)[-1]
          print(output.keys())
          print(output["hm"].dtype)
          print(output["reg"].dtype)
          print(output["tracking"].dtype)
          print(output["wh"].dtype)
      elif self.modeltype == "onnx":
          outputs = self.ort_session.run(None, {self.input_name_1 : images.cpu().numpy(), self.input_name_2: pre_images.cpu().numpy()})
          output  = {}
          output["hm"] = torch.tensor(outputs[0]).type(torch.float32)
          output["reg"] = torch.tensor(outputs[1]).type(torch.float32)
          output["tracking"] = torch.tensor(outputs[2]).type(torch.float32)
          output["wh"] = torch.tensor(outputs[3]).type(torch.float32)
      else:
          assert 1==0
      # ---------------------- save the output here ------------------------------
      numpy_hm = output["hm"].detach().cpu().numpy()
      #np.save("{}_hm.npy".format(self.cnt), numpy_hm)
      # print("############################",output.keys())
      # print(output["tracking"])
      # -----------------------------------!!!!!!!!!!!!!
      output = self._sigmoid_output(output)
      sigmoid_numpy_hm = output["hm"].detach().cpu().numpy()
      #np.save("{}_sigmoid_hm.npy".format(self.cnt), sigmoid_numpy_hm)
      #print("max : " ,np.max(sigmoid_numpy_hm))
      # print("############################",output.keys())
      # print(output["hm"])
  ...
  def show_results(self, debugger, image, results):
    ...
    if self.opt.debug == 4:
      print("self.opt.debug_dir ",self.opt.debug_dir)
