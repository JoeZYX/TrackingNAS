...
class GenericLoss(torch.nn.Module):
  ...
  def forward(self, outputs, batch):
    ...
    for s in range(opt.num_stacks): # opt.num_stacks = 1
      output = outputs[s]
      output = self._sigmoid_output(output)

      if 'hm' in output:
        # self.crit = FastFocalLoss
        # output['hm']  number of class x  outputw  x  outputh
        losses['hm'] += self.crit(
...
class Trainer(object):
  ...
  def run_epoch(self, phase, epoch, data_loader, trainig_configs=None):
    ...
    if trainig_configs and epoch in trainig_configs.keys():
        random_flag = False
        print(" Setting :  run_depth {} kernel {}".format(trainig_configs[epoch][1],trainig_configs[epoch][2]))
        model_with_loss.model.set_current_runtime_depth_and_kernel(
            current_run_time_depth=trainig_configs[epoch][1],
            kernel_index=trainig_configs[epoch][2],
            random=False,
            all_layers=False,
            active_net=False,
  ...
  def train(self, epoch, data_loader, trainig_configs = None):
    return self.run_epoch('train', epoch, data_loader, trainig_configs)
